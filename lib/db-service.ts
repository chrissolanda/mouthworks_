...
export const appointmentService = {
  ...
  async create(appointment: any) {
    try {
      ...
      fetch('http://127.0.0.1:7242/ingest/c0a6aa0c-74d6-4100-87e9-5e0b60c6253b',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'lib/db-service.ts:224',message:'DB service - before insert',data:{appointmentDate:appointment.date,today,patientId:appointment.patient_id},timestamp:Date.now(),sessionId:'debug-session',runId:'appointment-date-fix',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/c0a6aa0c-74d6-4100-87e9-5e0b60c6253b',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'lib/db-service.ts:235',message:'DB service - after insert',data:{submittedDate:appointment.date,savedDate:data?.date,today,patientName:data?.patients?.name},timestamp:Date.now(),sessionId:'debug-session',runId:'appointment-date-fix',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
...
export const paymentService = {
  ...
  async getByDentistId(dentistId: string) {
    try {
      ...
      const { data, error } = await getSupabase()
        .from("payments")
        .select("*, patients(name, email), appointments(status)")
        .eq("dentist_id", dentistId)
        .order("date", { ascending: false })
      if (error) {
  ...
  async getDentistEarnings(dentistId: string) {
    try {
      ...
      data?.forEach((payment: any) => {
        ...
        // SECONDARY: Also check appointment status as fallback
      ...
      console.log("[v0] Final earnings:", { totalEarned, totalPending, totalCompleted, count: data?.length || 0 })
    ...
    try {
      ...
      try {
        ...
        token = sessionResp?.data?.session?.access_token || 
                sessionResp?.session?.access_token || 
                sessionResp?.access_token
...
export const treatmentRecordService = {
  ...
  async getByDentistId(dentistId: string) {
    ...
    try {
      const { data, error } = await getSupabase()
        .from("treatment_records")
        .select("*, patients(name), treatments(name, category, price), dentists(name)")
        .eq("dentist_id", dentistId)
        .order("date", { ascending: false })
